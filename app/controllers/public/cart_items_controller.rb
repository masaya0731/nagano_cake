class Public::CartItemsController < ApplicationController
  ...
  def create
    ...
    if same_cart_item.blank?
      ...
      new_cart_item.save
      flash[:notice] = "新たに#{new_cart_item.item.name}をカートに入れました"
    else
      # 同じ商品があれば、その商品の個数に数を加算する
      same_cart_item.update(count: same_cart_item.count + cart_item_params[:count].to_i)
      flash[:notice] = "#{same_cart_item.item.name}の数を#{cart_item_params[:count]}個増やしました"
    end
  ...
  def update
    ...
    flash[:notice] = "#{cart_item.item.name}の数を#{cart_item_params[:count]}個に変更しました"
    redirect_to public_cart_items_path
  end
